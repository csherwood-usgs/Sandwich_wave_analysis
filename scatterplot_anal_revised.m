...
xg_b = nan*ones(nb,nrun);
Pb_b = nan*ones(nb,nrun);
Pb_sb = nan*ones(nb,nrun);
Pbx_b = nan*ones(nb,nrun);
Pbx_sb = nan*ones(nb,nrun);
dPbx_b = nan*ones(nb,nrun);
dPbx_sb = nan*ones(nb,nrun);
hsb_b = nan*ones(nb,nrun);
hsb_sb = nan*ones(nb,nrun);
R2b_b = nan*ones(nb,nrun);
R2b_sb = nan*ones(nb,nrun);
slopeb_b = nan*ones(nb,nrun);
slopeb_sb = nan*ones(nb,nrun);
for i=1:nb
   ...
   for j=1:nrun
   Pbn(:,j) = mrun(j).Pb.*gn;
   Pbxn(:,j) = mrun(j).Pb.*dalongb.*gn;
   dPbxn(:,j) = [0; -diff(mrun(j).Pb)].*dalongb.*gn;
   R2bxn(:,j) = mrun(j).R2b.*gn;
   hsbxn(:,j) = mrun(j).hsb.*gn;
   end
   Pb_b(i) = nanmean(Pbn(iok,:),'all');
   Pb_sb(i) = nanstd(Pbn(iok,:),0,'all');

   Pbx_b(i) = nanmean(Pbxn(iok,:),'all');
   Pbx_sb(i) = nanstd(Pbxn(iok,:),0,'all');

   dPbx_b(i) = nanmean(dPbxn(iok,:),'all');
   dPbx_sb(i) = nanstd(dPbxn(iok,:),0,'all');

   R2b_b(i) = nanmean(R2bxn(iok,:),'all');
   R2b_sb(i) = nanstd(R2bxn(iok,:),0,'all');

   hsb_b(i) = nanmean(hsbxn(iok,:),'all');
   hsb_sb(i) = nanstd(hsbxn(iok,:),0,'all');
end
isarmor = (xg_b<=400);
...
text(.75,.1,ts,'Fontsize',12,'Units','normalized')
ts = sprintf('{p} = %.3f',LM.Coefficients.pValue(2))
text(.75,.05,ts,'Fontsize',12,'Units','normalized')
set(gca, 'fontsize', 12)
...
print('vol_Px_scatter.png', '-dpng', '-r300')
%%
fig=figure(22);clf
fig.PaperUnits = 'inches'
fig.PaperPosition = [ 0 0 9 9 ];
...
print('vol_R2_scatter.png', '-dpng', '-r300')
%% R2 and Hs v. shoreline change
fig=figure(23);clf
fig.PaperUnits = 'inches'
fig.PaperPosition = [ 0 0 9 9 ];
% R2b v total vol change
subplot(221)
for i=1:length(R2b_b)
plot([R2b_b(i)-R2b_sb(i),R2b_b(i)+R2b_sb(i)],[mwlr_b(i), mwlr_b(i)],'-','color',[.6 .6 .6])
hold on
plot([R2b_b(i), R2b_b(i)],[mwlr_b(i)-mwlr_sb(i), mwlr_b(i)+mwlr_sb(i)],'-','color',[.6 .6 .6])
end
h1=plot(R2b_b(isarmor),mwlr_b(isarmor),'.k','markersize',25);
h2=plot(R2b_b(isdune),mwlr_b(isdune),'.r','markersize',25);
h3=plot(R2b_b(iseast),mwlr_b(iseast),'.b','markersize',25);
ylabel('MWL Shoreline Change [m]','fontsize',14)
text(.02,.95,'a','Fontsize',14,'Units','normalized')
LM = fitlm(R2b_b(isdune),mwlr_b(isdune))
ts = sprintf('{r}^2 = %.3f',LM.Rsquared.Ordinary)
text(.75,.1,ts,'Fontsize',12,'Units','normalized')
ts = sprintf('{p} = %.3f',LM.Coefficients.pValue(2))
text(.75,.05,ts,'Fontsize',12,'Units','normalized')
...
subplot(222)
for i=1:length(hsb_b)
plot([hsb_b(i)-hsb_sb(i),hsb_b(i)+hsb_sb(i)],[mwlr_b(i), mwlr_b(i)],'-','color',[.6 .6 .6])
hold on
plot([hsb_b(i), hsb_b(i)],[mwlr_b(i)-mwlr_sb(i), mwlr_b(i)+mwlr_sb(i)],'-','color',[.6 .6 .6])
end
h1=plot(hsb_b(isarmor),mwlr_b(isarmor),'.k','markersize',25);
h2=plot(hsb_b(isdune),mwlr_b(isdune),'.r','markersize',25);
h3=plot(hsb_b(iseast),mwlr_b(iseast),'.b','markersize',25);
text(.02,.95,'b','Fontsize',14,'Units','normalized')
LM = fitlm(hsb_b(isdune),mwlr_b(isdune))
ts = sprintf('{r}^2 = %.3f',LM.Rsquared.Ordinary)
text(.75,.1,ts,'Fontsize',12,'Units','normalized')
ts = sprintf('{p} = %.3f',LM.Coefficients.pValue(2))
text(.75,.05,ts,'Fontsize',12,'Units','normalized')
...
subplot(223)
for i=1:length(R2b_b)
plot([R2b_b(i)-R2b_sb(i),R2b_b(i)+R2b_sb(i)],[mhwr_b(i), mhwr_b(i)],'-','color',[.6 .6 .6])
hold on
plot([R2b_b(i), R2b_b(i)],[mhwr_b(i)-mhwr_sb(i), mhwr_b(i)+mhwr_sb(i)],'-','color',[.6 .6 .6])
end
h1=plot(R2b_b(isarmor),mhwr_b(isarmor),'.k','markersize',25);
h2=plot(R2b_b(isdune),mhwr_b(isdune),'.r','markersize',25);
h3=plot(R2b_b(iseast),mhwr_b(iseast),'.b','markersize',25);
ylabel('MHW Shoreline Change [m]','fontsize',14)
xlabel('{\itR}_2 [m]','fontsize',14)
ylim([-15,5])
LM = fitlm(R2b_b(isdune),mhwr_b(isdune))
ts = sprintf('{r}^2 = %.3f',LM.Rsquared.Ordinary)
text(.75,.1,ts,'Fontsize',12,'Units','normalized')
ts = sprintf('{p} = %.3f',LM.Coefficients.pValue(2))
text(.75,.05,ts,'Fontsize',12,'Units','normalized')
...
subplot(224)
for i=1:length(hsb_b)
plot([hsb_b(i)-hsb_sb(i),hsb_b(i)+hsb_sb(i)],[mhwr_b(i), mhwr_b(i)],'-','color',[.6 .6 .6])
hold on
plot([hsb_b(i), hsb_b(i)],[mhwr_b(i)-mhwr_sb(i), mhwr_b(i)+mhwr_sb(i)],'-','color',[.6 .6 .6])
end
h1=plot(hsb_b(isarmor),mhwr_b(isarmor),'.k','markersize',25);
h2=plot(hsb_b(isdune),mhwr_b(isdune),'.r','markersize',25);
h3=plot(hsb_b(iseast),mhwr_b(iseast),'.b','markersize',25);
ylim([-15,5])
xlabel('{\itH_s} [m]','fontsize',14)
text(.02,.95,'d','Fontsize',14,'Units','normalized')
LM = fitlm(hsb_b(isdune),mhwr_b(isdune))
ts = sprintf('{r}^2 = %.3f',LM.Rsquared.Ordinary)
text(.75,.1,ts,'Fontsize',12,'Units','normalized')
ts = sprintf('{p} = %.3f',LM.Coefficients.pValue(2))
text(.75,.05,ts,'Fontsize',12,'Units','normalized')
...
print('shoreline_R2_scatter.png', '-dpng', '-r300')
%% Px and delPx v. shoreline change
fig=figure(23);clf
fig.PaperUnits = 'inches'
fig.PaperPosition = [ 0 0 9 9 ];

% dPBx v MWL shoreline
subplot(221)
for i=1:length(Pbx_b)
plot([Pbx_b(i)-Pbx_sb(i),Pbx_b(i)+Pbx_sb(i)],[mwlr_b(i), mwlr_b(i)],'-','color',[.6 .6 .6])
hold on
plot([Pbx_b(i), Pbx_b(i)],[mwlr_b(i)-mwlr_sb(i), mwlr_b(i)+mwlr_sb(i)],'-','color',[.6 .6 .6])
end
h1=plot(Pbx_b(isarmor),mwlr_b(isarmor),'.k','markersize',25);
h2=plot(Pbx_b(isdune),mwlr_b(isdune),'.r','markersize',25);
h3=plot(Pbx_b(iseast),mwlr_b(iseast),'.b','markersize',25);
ylabel('MWL Shoreline Change [m]','fontsize',14)
text(.02,.95,'a','Fontsize',14,'Units','normalized')
LM = fitlm(Pbx_b(isdune),mwlr_b(isdune))
ts = sprintf('{r}^2 = %.3f',LM.Rsquared.Ordinary)
text(.75,.1,ts,'Fontsize',12,'Units','normalized')
ts = sprintf('{p} = %.3f',LM.Coefficients.pValue(2))
text(.75,.05,ts,'Fontsize',12,'Units','normalized')
...
subplot(222)
for i=1:length(dPbx_b)
plot([dPbx_b(i)-dPbx_sb(i),dPbx_b(i)+dPbx_sb(i)],[mwlr_b(i), mwlr_b(i)],'-','color',[.6 .6 .6])
hold on
plot([dPbx_b(i), dPbx_b(i)],[mwlr_b(i)-mwlr_sb(i), mwlr_b(i)+mwlr_sb(i)],'-','color',[.6 .6 .6])
end
h1=plot(dPbx_b(isarmor),mwlr_b(isarmor),'.k','markersize',25);
h2=plot(dPbx_b(isdune),mwlr_b(isdune),'.r','markersize',25);
h3=plot(dPbx_b(iseast),mwlr_b(iseast),'.b','markersize',25);
%xlim([-.1 .1])
text(.02,.95,'b','Fontsize',14,'Units','normalized')
LM = fitlm(dPbx_b(isdune),mwlr_b(isdune))
ts = sprintf('{r}^2 = %.3f',LM.Rsquared.Ordinary)
text(.75,.1,ts,'Fontsize',12,'Units','normalized')
ts = sprintf('{p} = %.3f',LM.Coefficients.pValue(2))
text(.75,.05,ts,'Fontsize',12,'Units','normalized')
...
subplot(223)
for i=1:length(Pbx_b)
plot([Pbx_b(i)-Pbx_sb(i),Pbx_b(i)+Pbx_sb(i)],[mhwr_b(i), mhwr_b(i)],'-','color',[.6 .6 .6])
hold on
plot([Pbx_b(i), Pbx_b(i)],[mhwr_b(i)-mhwr_sb(i), mhwr_b(i)+mhwr_sb(i)],'-','color',[.6 .6 .6])
end
h1=plot(Pbx_b(isarmor),mhwr_b(isarmor),'.k','markersize',25);
h2=plot(Pbx_b(isdune),mhwr_b(isdune),'.r','markersize',25);
h3=plot(Pbx_b(iseast),mhwr_b(iseast),'.b','markersize',25);
ylabel('MHW Shoreline Change [m]','fontsize',14)
xlabel('{\itP_x}  [kW m^{-1}]','fontsize',14)
ylim([-15,5])
LM = fitlm(Pbx_b(isdune),mhwr_b(isdune))
ts = sprintf('{r}^2 = %.3f',LM.Rsquared.Ordinary)
text(.75,.1,ts,'Fontsize',12,'Units','normalized')
ts = sprintf('{p} = %.3f',LM.Coefficients.pValue(2))
text(.75,.05,ts,'Fontsize',12,'Units','normalized')
...
% dPbx v MHW shoreline
subplot(224)
for i=1:length(dPbx_b)
plot([dPbx_b(i)-dPbx_sb(i),dPbx_b(i)+dPbx_sb(i)],[mhwr_b(i), mhwr_b(i)],'-','color',[.6 .6 .6])
hold on
plot([dPbx_b(i), dPbx_b(i)],[mhwr_b(i)-mhwr_sb(i), mhwr_b(i)+mhwr_sb(i)],'-','color',[.6 .6 .6])
end
h1=plot(dPbx_b(isarmor),mhwr_b(isarmor),'.k','markersize',25);
h2=plot(dPbx_b(isdune),mhwr_b(isdune),'.r','markersize',25);
h3=plot(dPbx_b(iseast),mhwr_b(iseast),'.b','markersize',25);
%xlim([-.1 .1])
ylim([-15,5])
xlabel('-{\Delta} {\itP_x}/{\Delta}{\itx}  [kW m^{-2}]','fontsize',14)
text(.02,.95,'d','Fontsize',14,'Units','normalized')
LM = fitlm(dPbx_b(isdune),mhwr_b(isdune))
ts = sprintf('{r}^2 = %.3f',LM.Rsquared.Ordinary)
text(.75,.1,ts,'Fontsize',12,'Units','normalized')
ts = sprintf('{p} = %.3f',LM.Coefficients.pValue(2))
text(.75,.05,ts,'Fontsize',12,'Units','normalized')
...
print('shoreline_Px_scatter.png', '-dpng', '-r300')